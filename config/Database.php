...
class Database
{
    ...
    private function configurarConexao(): array
    {
        $url = $this->getEnv('MYSQL_URL', $this->getEnv('DATABASE_URL'));
        ...
        if ($url !== '') {
            ...
            if ($partes !== false && isset($partes['host'])) {
                return [
                    ...
                    'dbname' => ltrim((string) ($partes['path'] ?? '/pets_e_rabiscos'), '/'),
                    'user' => rawurldecode((string) ($partes['user'] ?? '')),
